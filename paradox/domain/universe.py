...
class UniverseAssetManager(ABC, BaseModel):
...
class FileUniverseAssetManager(UniverseAssetManager):
    universes: dict[str, Universe] = Field(default={})
    universes_path: Path = Field(default=Path("assets/universes"))

    def initialize(self, universes_path: Path = Path("assets/universes")) -> None:
...
universe_assets = FileUniverseAssetManager()
